...
export const NotarySignatureWorkarea: React.FC = () => {
  ...
  const navigationForceSaveRequestId = (location.state as any)?.forceSaveRequestId as string | null | undefined;
  const navigationRequestedAfterMsRaw = (location.state as any)?.requestedAfterMs as number | string | null | undefined;
  const navigationPendingLatestSync = !!(location.state as any)?.pendingLatestSync;
  const initialPreferredUrl = String(navigationFinalPdfUrl || '').trim() || null;
  const navigationRequestedAfterMs =
    typeof navigationRequestedAfterMsRaw === 'number'
      ? navigationRequestedAfterMsRaw
      : Number(navigationRequestedAfterMsRaw || 0) || 0;
  ...
  const [awaitingLatestSync, setAwaitingLatestSync] = useState(navigationPendingLatestSync);
  ...
  const viewerPdfObjectUrlRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    return () => {
      if (viewerPdfObjectUrlRef.current) {
        URL.revokeObjectURL(viewerPdfObjectUrlRef.current);
        viewerPdfObjectUrlRef.current = null;
      }
    };
  }, []);
  ...
  const { data: rasm, isLoading } = trpc.feesAgent.documents.getSavedRasm.useQuery(
    { sessionToken: sessionToken || '', id: id || '' },
    {
      ...
      refetchInterval: awaitingLatestSync ? 1000 : false,
    }
  );
  ...
  const toStrictPdfUrl = (rawUrl?: string | null, rawName?: string | null, rawMime?: string | null) => {
    const value = String(rawUrl || '').trim();
    if (!value) return null;
    return isPdfCandidate(value, rawName, rawMime) ? value : null;
  };

  const pickBestSigningCandidate = React.useCallback(() => {
    const attachments = Array.isArray(rasm?.attachments) ? rasm.attachments : [];
    const latestVersionId = (rasm as any)?.latestDraftVersionId ? String((rasm as any).latestDraftVersionId) : null;
    const latestDraftDocxUrl = String((rasm as any)?.latestDraftDocxUrl || '').trim() || null;

    const normalizeVersionId = (attachment: any) =>
      String(attachment?.metadata?.versionId || attachment?.metadata?.version_id || '').trim() || null;

    const pdfAttachments = attachments.filter((attachment: any) =>
      isPdfCandidate(attachment?.fileUrl, attachment?.fileName, attachment?.mimeType || attachment?.type),
    );
    const docxAttachments = attachments.filter((attachment: any) => {
    ...
    const pickMainPdf = () => {
      ...
      for (const category of priority) {
        const exact = pdfAttachments.find((attachment: any) => {
          const attachmentCategory = String(attachment?.category || '').toLowerCase();
          return attachmentCategory === category && latestVersionId && normalizeVersionId(attachment) === latestVersionId;
        });
        if (exact) return exact;
      }
      for (const category of priority) {
        const fallback = pdfAttachments.find((attachment: any) => String(attachment?.category || '').toLowerCase() === category);
    ...
    const pickDocx = () => {
      const exact = docxAttachments.find((attachment: any) => latestVersionId && normalizeVersionId(attachment) === latestVersionId);
      if (exact) return {
        url: String(exact.fileUrl || '').trim() || null,
        versionId: normalizeVersionId(exact) || latestVersionId,
        attachmentId: exact.id ? String(exact.id) : null,
      };
      ...
      const fallback = docxAttachments[0];
      if (!fallback) return null;
      return {
        url: String(fallback.fileUrl || '').trim() || null,
        versionId: normalizeVersionId(fallback) || latestVersionId,
        attachmentId: fallback.id ? String(fallback.id) : null,
      };
    };

    const mainPdf = pickMainPdf();
    if (mainPdf) {
      return {
        kind: 'pdf' as const,
        url: String(mainPdf.fileUrl || '').trim() || null,
        versionId: normalizeVersionId(mainPdf) || latestVersionId,
        attachmentId: mainPdf.id ? String(mainPdf.id) : null,
      };
    }
    ...
    if (docx?.url) {
      return {
        kind: 'docx' as const,
        url: docx.url,
        versionId: docx.versionId || latestVersionId,
        attachmentId: docx.attachmentId,
      };
    }

    return null;
  }, [awaitingLatestSync, navigationPendingLatestSync, rasm]);
  ...
  useEffect(() => {
    ...
    setPage(1);
    setAwaitingLatestSync(navigationPendingLatestSync);
    setSelectedAttachmentId(null);
    const seededPdfUrl = toStrictPdfUrl(initialPreferredUrl);
    if (!seededPdfUrl) return;
    const seededUrl = appendCacheBuster(seededPdfUrl, 'nav', String(Date.now()));
    setOriginalPdfUrl(seededUrl);
    setCurrentPdfUrl(seededUrl);
  }, [initialPreferredUrl, navigationPendingLatestSync]);

  useEffect(() => {
    if (!awaitingLatestSync) return;
    const timeout = window.setTimeout(() => {
      setAwaitingLatestSync(false);
    }, 6000);
  ...
  useEffect(() => {
    ...
    const payloadForceSaveRequestId =
      String((rasm as any)?.payload?.latestForceSaveRequestId || '').trim() || null;
    const requestMatched =
      !!navigationForceSaveRequestId && payloadForceSaveRequestId === navigationForceSaveRequestId;
    const updatedAfterRequest =
      !!navigationRequestedAfterMs &&
      !!(rasm as any)?.latestDraftUpdatedAt &&
      Date.parse(String((rasm as any).latestDraftUpdatedAt)) >= navigationRequestedAfterMs - 1000;
    const candidate = pickBestSigningCandidate();

    if ((requestMatched || updatedAfterRequest) && !!candidate?.url) {
      setAwaitingLatestSync(false);
    }
  }, [
    awaitingLatestSync,
    navigationForceSaveRequestId,
    navigationRequestedAfterMs,
    pickBestSigningCandidate,
    rasm,
  ]);
  ...
  useEffect(() => {
    ...
    const loadPdfMeta = async () => {
      try {
        ...
        viewerPdfObjectUrlRef.current = objectUrl;
  ...
  useEffect(() => {
    ...
    if (candidate.attachmentId) {
      setSelectedAttachmentId((prev) => (prev === candidate.attachmentId ? prev : candidate.attachmentId));
    }

    if (candidate.kind !== 'pdf') return;

    const stableCacheValue =
      candidate.versionId ||
      String((rasm as any)?.latestDraftUpdatedAt || '') ||
      candidate.attachmentId ||
      'latest';
    const sourceKey = `${candidate.url}::${stableCacheValue}`;
    if (appliedSourceKeyRef.current === sourceKey) return;

    const busted = appendCacheBuster(candidate.url, candidate.versionId ? 'v' : 'u', stableCacheValue);
    appliedSourceKeyRef.current = sourceKey;

    setOriginalPdfUrl(busted);
    setCurrentPdfUrl(busted);
    setEditedPdfBytes(null);
  }, [pickBestSigningCandidate, rasm]);

  useEffect(() => {
    const candidate = pickBestSigningCandidate();
    const fallbackVersionId = (rasm as any)?.latestDraftVersionId ? String((rasm as any).latestDraftVersionId) : null;
    const versionIdToFinalize =
      (candidate?.kind === 'docx' && candidate.versionId) ||
      fallbackVersionId ||
      null;
    const currentUrl = String(currentPdfUrl || originalPdfUrl || '').trim();
    const currentUrlIsPdf = !!currentUrl && isPdfCandidate(currentUrl, '', '');

    if (!versionIdToFinalize || !sessionToken) return;
    if (candidate?.kind === 'pdf' && currentUrlIsPdf) return;
  ...
  }, [currentPdfUrl, finalizeForSigningMutation, id, originalPdfUrl, pickBestSigningCandidate, rasm, sessionToken, trpcUtils]);
  ...
  useEffect(() => {
    ...
    if (url && isPdfCandidate(url, selectedAttachment?.fileName, selectedAttachment?.mimeType || selectedAttachment?.type)) {
      const stableCacheValue =
        String(selectedAttachment?.metadata?.versionId || selectedAttachment?.metadata?.version_id || '').trim() ||
        String(selectedAttachment?.id || '') ||
        'attachment';
      const sourceKey = `${url}::${stableCacheValue}`;
      if (appliedSourceKeyRef.current === sourceKey) return;
      const busted = appendCacheBuster(url, 'a', stableCacheValue);
      appliedSourceKeyRef.current = sourceKey;
      setOriginalPdfUrl(busted);
      setCurrentPdfUrl(busted);
      setEditedPdfBytes(null);
  ...
  }, [selectedAttachmentId, rasm?.attachments]);
  ...
  const pdfRenderUrl = viewerPdfUrl || '';
  const fileName = (attachment?.fileName || '').toLowerCase();
  const fileMime = String(attachment?.mimeType || attachment?.type || '').toLowerCase();
  const displayedFileName = (currentPdfUrl || originalPdfUrl) ? '' : fileName;
  const displayedFileMime =
    (currentPdfUrl || originalPdfUrl)
      ? 'application/pdf'
      : fileMime;

  const isPDF = isPdfCandidate(url, displayedFileName, displayedFileMime);
        ...
        <div className="flex-[3] flex flex-col border-l border-stone-400/60 bg-stone-300 relative h-full overflow-hidden">
          ...
          <div className="flex-1 overflow-auto overscroll-contain bg-stone-300 flex items-start justify-center py-6">
            ...
            >
              ...
              >
                {awaitingLatestSync && (
                  <div className="absolute inset-0 z-[120] flex items-center justify-center bg-white/88 backdrop-blur-sm">
                    <div className="rounded-3xl border border-slate-200 bg-white px-8 py-7 shadow-2xl text-center">
                      <div className="mx-auto mb-4 h-10 w-10 rounded-full border-4 border-blue-500 border-t-transparent animate-spin" />
                      <p className="text-lg font-black text-slate-900">جاري جلب النسخة الأحدث للتوقيع</p>
                      <p className="mt-2 text-sm font-bold text-slate-500">انتظار قصير لمزامنة آخر تعديل من AuditHub...</p>
                    </div>
                  </div>
                )}
                  ...
                  <>
                    {isPDF && pdfRenderUrl ? (
                      <div className="w-full h-full bg-white">
                        <iframe
                          key={buildPdfViewerSrc(pdfRenderUrl, { page })}
                          src={buildPdfViewerSrc(pdfRenderUrl, { page })}
                      ...
                      <div className="w-full h-full flex flex-col items-center justify-center text-slate-500 bg-white px-8 text-center">
                        ...
                        <p className="font-black text-lg">جاري تحميل ملف PDF للعرض</p>
                        <p className="text-sm mt-2">يتم تجهيز نسخة صالحة للعرض داخل مساحة التوقيع.</p>
                      </div>
